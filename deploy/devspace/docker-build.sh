...
if [[ "${DEVSPACE_DEBUG}" == "--debug=delve" ]]; then
    time docker buildx build --progress plain --output "type=docker" --load --platform=linux/amd64 -f ${DOCKER_FILE} --target image-debug --build-arg GCFLAGS='all=-N -l' -t "${DOCKER_IMAGE}" .
    ...
    time docker buildx build --progress plain --output "type=docker" --load --platform=linux/amd64 -f ${DOCKER_FILE} -t "${DOCKER_IMAGE}" .
